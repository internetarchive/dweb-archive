...
function pushHistory(...optss) {
  ...
  const opts = ObjectFilter(opts, (k,v) => (typeof v !== "undefined"));
  if (opts.wanthistory) {
    ...
    const optsDetails = ObjectFilter(opts, (k,v) => !optsInDetailsUrl.includes(k));
    ...
    url.path =  (!supportsArc) ? window.location.pathname : query ? '/arc/archive.org/details' : `/arc/archive.org/${opts.download ? "download" : "details"}${identifier ? "/" + identifier : ""}${page ? "/page/" + page : ""}`
...
export default class Nav {
  constructor() {
    ...
    static async nav_search(q, opts={}) {
        ...
        opts = pushHistory(opts, {query: s.query}); // Pass
    ...
    static async factory(identifier, ...optss) {
      ...
      */
        ...
        const {wanthistory=true, download=undefined, page=undefined, noCache=undefined} = opts;
        const destn = document.getElementById('main'); // Blank window (except Nav) as loading
        Nav.clear(destn);
        ...
        try {
            if (!identifier || (identifier === "home")) {
                ...
                if (!d.metadata) {
                    new DetailsError({itemid: identifier, message: `item ${identifier} cannot be found or does not have metadata`}).render(destn);
                } else {
                    if (d.metadata.title) {
  ...
  static Nav.setState(...optss) {
    ...
    this.state = Object.filter(combinedOpts, (k,v) => persistentState.includes(k)); // Dont mind keeping undefined state
    return Object.filter(combinedOpts, (k,v) => !persistentState.includes(k)); // return any opts not persistent
  }
}
